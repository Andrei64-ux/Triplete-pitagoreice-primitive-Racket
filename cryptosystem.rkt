...
(define (my_list_helper e f)
  (list (a1 e f) (b1 e f) (c1 e f) (a2 e f) (b2 e f) (c2 e f) (a3 e f) (b3 e f) (c3 e f)))

(define (key n)
  (let* ((result (get-nth-quadruple n))
    (e (cadr result))
    (f (caddr result))
    )
      (map (lambda (a)
          (modulo a 27)) (my_list_helper e f))))  
...
(define (message->codes message)
  (map (lambda (x) (modulo (- (char->integer x) 32) 32)) (string->list message)))
...
(define (codes->message codes)
  (list->string (map (lambda (x)
                       (if (= x 0) (integer->char 32) (integer->char (+ x 96)))) codes))
  )
...
(define (extend-key key size)
  (let ((n (length key)))
    (if(> size n)
       (append key (extend-key key (- size n)))
       (take key size))))
...
(define (my_coding_list op)
    (lambda (message key)
        (let ((n (length message)))
          (let ((partial_result (map op message (extend-key key n))))
            (map (lambda(x)
                   (modulo x 27)) partial_result)))))
...
(define (my_coding_string op)
    (lambda (message key)
        (codes->message (op (message->codes message) key))))

(define encrypt-message
  (my_coding_string encrypt-codes))

(define decrypt-message
  (my_coding_string decrypt-codes))
