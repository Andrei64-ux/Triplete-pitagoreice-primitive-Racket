...
(define (get-transformations-helper n)
  (cond ((= 1 n) '())
        ((= (modulo n 3) 0) (append (list 2) (get-transformations-helper (quotient n 3))))
        ((= (modulo n 3) 1) (append (list 3) (get-transformations-helper (quotient n 3))))
        ((= (modulo n 3) 2) (append (list 1) (get-transformations-helper (+ 1 (quotient n 3)))))
    ))

(define (get-transformations n)
  (reverse (get-transformations-helper n)))
...
; Formam lista de transformari (fie de T-uri , fie de Q-uri )
; pornind de la lista numerica de transformari a lui n .
; Aplicam acum lista finala de transformari pe tuplu ,
; folosindu-ne de 'apply-transf' care in functie de caz
; rezolva problema pt triplete , fie cvartete.
(define (get-nth-tuple tuple Fs apply-transf)
      (lambda (n)
        (foldl apply-transf tuple
               (map (lambda (x)
                      (cond
                        ((= x 1) (car Fs))
                        ((= x 2) (cadr Fs))
...
(define get-nth-ppt-from-matrix-transformations 
  (get-nth-tuple '(3 4 5) (list T1 T2 T3) (lambda (matrix res) (multiply matrix res))))
...
(define get-nth-quadruple
  (get-nth-tuple '(1 1 2 3) (list Q1 Q2 Q3) (lambda (transformation res) (apply transformation res))))
