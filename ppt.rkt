...
(define (tail-recursion M V acc)
  (if (null? M)
      acc
      (tail-recursion (cdr M) V (append acc (list (dot-product (car M) V))))))
...
(define (apply-matrix-transformations Ts ppt)
  (cond ((null? Ts) ppt)
       ((equal? 1 (car Ts)) (apply-matrix-transformations (cdr Ts) (multiply T1 ppt)))
       ((equal? 2 (car Ts)) (apply-matrix-transformations (cdr Ts) (multiply T2 ppt)))
       ((equal? 3 (car Ts)) (apply-matrix-transformations (cdr Ts) (multiply T3 ppt)))))
