...
(define my_stream
  (lambda (s)
    (let ((first_tuple (stream-first s)))
      (stream-cons
       first_tuple
       (my_stream (stream-rest s))
       ))))

(define ppt-stream-in-tree-order
  (let ((queue-stream (make-queue (stream->list (stream '(3 4 5))))))
  (my_stream queue-stream)))
...
; am creat functiile pe baza algoritmului descris la
; https://berkeley-cs61as.github.io/textbook/example-infinite-streams-of-pairs.html

(define (interleave G H)
  (let ((g (stream-first G))
        (h (stream-first H)))
    (if (> (cdr g) (cdr h))
        (stream-cons h (interleave G (stream-rest H)))
        (stream-cons g (stream-cons h (interleave (stream-rest G) (stream-rest H))))))
  )
  

(define (pairs G H)
  (stream-cons (cons (stream-first G) (stream-first H))
               (interleave (stream-map (lambda (x) (cons (stream-first G) x))
                                       (stream-rest H))
